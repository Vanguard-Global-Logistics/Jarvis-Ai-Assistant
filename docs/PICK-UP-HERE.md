...
| HEAD at handoff       | Forge v1 (ADR 0034) and Ledger v1 (ADR 0035) both shipped — twenty-nine IPC channels, ten migrations, thirty-five ADRs. **Ledger's mandatory independent review has NOT been sent** (`docs/review/review-ledger.md` is written and ready). Its panel now writes — the read-only gap recorded here earlier is closed. |
...
| **Ledger v1** — Safe-to-Spend that REFUSES to compute rather than treat unknown as zero; purchase reviews; cannot move money | Store + IPC IMPLEMENTED AND VERIFIED on `probe:runtime` (ADR 0035). Panel READS AND WRITES — ENTER FIGURES and OPEN A REVIEW, with `probe:runtime` driving a typed figure through the form into real SQLite. Cost Governor still unwired. **Independent review OUTSTANDING and mandatory.** |
