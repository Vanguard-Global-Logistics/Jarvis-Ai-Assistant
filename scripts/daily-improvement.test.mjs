...
describe('daily continual-improvement policy', () => {
  it('creates a proposed skill candidate after three repetitions in seven days', () => {
    const report = buildDailyImprovementReport(
      [observation({ id: 'one' }), observation({ id: 'two' }), observation({ id: 'three' })],
  ...
  it('rejects secret-like material and unsupported transcript fields', () => {
    expect(() => observation({ summary: 'password=do-not-store-this' })).toThrow(/secret/);
    expect(() =>
      validateObservation({
        ...observation(),
  ...
  it('fails closed on malformed JSONL instead of silently dropping evidence', () => {
    expect(() => parseObservationJsonLines(`${JSON.stringify(observation())}\nnot-json\n`)).toThrow(
      /line 2/,
    );
  });
  ...
  it('writes private reports and never writes a promotion record', () => {
    ...
    for (const id of ['one', 'two', 'three']) appendObservation(eventPath, observation({ id }));
    ...
    const stored = JSON.parse(readFileSync(join(stateRoot, 'last-run.json'), 'utf8'));
