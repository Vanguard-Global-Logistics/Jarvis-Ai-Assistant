...
describe("Hermes shell entry points", () => {
  ...
  it("keeps the signed release manifest wired to the installer and doctor", () => {
    for (const script of [
      ...
      expect(readFileSync(resolve(root, script), "utf8")).toContain(
        "hermes-release.env",
      );
    }
  });
  ...
  it("prunes the retired STT toolset without disabling local transcription", () => {
    ...
    const config = readFileSync(
      resolve(root, "jarvis-hermes/config.yaml"),
      "utf8",
    );
  ...
  it("installs and verifies the owner-approved Octagon commercial memory", () => {
    ...
    expect(
      readFileSync(resolve(root, "jarvis-hermes", memoryPath), "utf8"),
    ).toContain(
  ...
  it("installs the personal Professional Mode identity without employer ownership", () => {
    ...
    const professionalMemory = readFileSync(
      resolve(root, "jarvis-hermes", memoryPath),
      "utf8",
    );
  ...
  it("installs the owner-approved job-mastery and field-progress roadmap", () => {
    ...
    expect(roadmap).toContain(
      "Do not estimate from technician headcount alone.",
    );
    expect(roadmap).toContain(
  ...
  it("installs the Coolify boundary without adopting BrainOutside as a dependency", () => {
    ...
    expect(adoption).toContain(
      "Coolify must run on a separate, supported Linux VPS.",
    );
    expect(adoption).toContain(
    ...
    expect(learning).toContain(
      "show an owner-visible diff or before/after preview",
    );
  ...
  it("pins reconstruction to the exact ordered R13.3 voice-loop parts", () => {
    ...
    const parts = [
      ...script.matchAll(/live_voice_loop\.parts\/(\d+\.py\.part)/g),
    ].map((match) => match[1]);
    ...
    for (const part of parts) {
      digest.update(
        readFileSync(
          resolve(
            root,
            "runtime/macos/voice-r13.3/live_voice_loop.parts",
            part,
          ),
        ),
      );
  ...
  it("uses uncertain non-wake speech as noisy-room evidence before rejection", () => {
    const source = readFileSync(
      resolve(
        root,
        "runtime/macos/voice-r13.3/live_voice_loop.parts/04.py.part",
      ),
      "utf8",
    );
  ...
  it("keeps sentence-ending Jarvis wake-only and behind close-owner rescue", () => {
    const parser = readFileSync(
      resolve(
        root,
        "runtime/macos/voice-r13.3/live_voice_loop.parts/03.py.part",
      ),
      "utf8",
    );
    ...
    const commandGate = readFileSync(
      resolve(
        root,
        "runtime/macos/voice-r13.3/live_voice_loop.parts/05.py.part",
      ),
      "utf8",
    );

    expect(parser).toContain(
      "2 <= len(tokens) <= 8 and tokens[-1].lower() == WAKE",
    );
    expect(routing).toContain("terminal_wake_handshake = True");
    expect(routing).toContain('inline = ""');
    expect(routing).toContain(
      "owner_score >= OWNER_TERMINAL_HANDSHAKE_THRESHOLD",
    );
    expect(routing).toContain(
      "nearfield_ratio >= OWNER_TERMINAL_HANDSHAKE_NEARFIELD",
    );
    expect(routing).toContain(
      "if not terminal_wake_handshake and is_end_session(text):",
    );
  ...
  it("keeps listening and captures the whole command after standalone Jarvis", () => {
    const config = readFileSync(
      resolve(
        root,
        "runtime/macos/voice-r13.3/live_voice_loop.parts/00.py.part",
      ),
      "utf8",
    );
    const capture = readFileSync(
      resolve(
        root,
        "runtime/macos/voice-r13.3/live_voice_loop.parts/02.py.part",
      ),
      "utf8",
    );
    const routing = readFileSync(
    ...
    expect(routing).toContain(
      "COMMAND_END_SILENCE_MS if owner_slot_waiting else None",
    );
    expect(routing).toContain(
      "OWNER_COMMAND_SLOT_UNTIL if owner_slot_waiting else None",
    );
  ...
  it("prewarms a representative first response for George", () => {
    const source = readFileSync(
      resolve(
        root,
        "runtime/macos/voice-r13.3/live_voice_loop.parts/01.py.part",
      ),
      "utf8",
    );

    expect(source).toContain(
      '("Yes?", "Standing by.", "Jarvis is ready to help.")',
    );
  });
});
